...
export default function AdminDashboardClient({ session }) {
  ...
  const [voidForm, setVoidForm] = useState({ void_reason: '' });
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      if ([membersRes.status, loansRes.status, paymentsRes.status].some((status) => status === 401)) {
  ...
  const handleVoidPayment = async (e) => {
    ...
    try {
      const res = await fetch(`/api/payments/${voidPaymentId}/void`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ void_reason: voidForm.void_reason }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to void transaction.');

      setVoidPaymentId(null);
      setVoidForm({ void_reason: '' });
      await fetchAllData();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <header className="border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <span className="hidden sm:block text-xs text-slate-500">{session.email}</span>
            ...
            <button
              ...
              className="py-1.5 px-3 rounded-lg border border-slate-800 bg-slate-950 text-slate-400 hover:text-slate-100 hover:bg-slate-900 transition-colors text-xs font-semibold cursor-pointer"
            >
              Sign Out
            </button>
          </div>
                    ...
                    {voidPaymentId && (
                      <form onSubmit={handleVoidPayment} className="p-6 rounded-2xl border border-rose-500/20 bg-rose-500/5 space-y-4">
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1">
                            <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">Authorized Actor</label>
                            <input
                              type="email"
                              value={session.email}
                              disabled
                              className="w-full px-3 py-2 rounded-lg border border-slate-800 bg-slate-900 text-slate-500 text-sm"
                            />
                          </div>
                        </div>
                        <button
                  ...
                  <div className="space-y-4">
                    ...
                    <form onSubmit={handleAddPayment} className="p-6 rounded-2xl border border-slate-900 bg-slate-900/40 space-y-4">
                      ...
                      <p className="text-xs text-slate-500">
                        Collector identity is recorded from your authenticated admin session: {session.email}
                      </p>
                      <button
                        type="submit"
                        disabled={actionLoading}
