...
- **Unified Front-End:** Visual dashboard panels for members to check balances, request loans, and read admin bulletins while admins manage ledger sheets.
...
#### Tab: `Admin_Bulletins`
`bulletin_id | message | status | created_at | updated_at`
...
#### Tab: `Loan_Requests`
`request_id | member_id | requested_amount | requested_term_months | preferred_release_date | purpose | status | reviewed_by | admin_notes | approved_interest_rate | approved_loan_id | created_at | updated_at`
...
- **Member loan request:** [http://localhost:3000/member-dashboard/loan-request](http://localhost:3000/member-dashboard/loan-request)
...
- **Admin bulletin and loan request review:** [http://localhost:3000/admin](http://localhost:3000/admin) (publish the member overview bulletin and approve or reject member loan requests)
